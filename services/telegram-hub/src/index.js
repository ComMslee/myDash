...
import { deleteMyCommands } from './telegram.js';
...
try {
  ...
  // 텔레그램 입력창 [/] 메뉴 비우기 — 봇은 Reply 키보드 진입만 사용.
  // 전역 default + root 개인 메뉴 모두 삭제 (이전 setMyCommands 잔재 청소).
  Promise.all([deleteMyCommands(), syncUserMenu(rootChatId)])
    .then(() => console.log('[boot] [/] menu cleared (Reply 키보드만 사용)'))
    .catch((e) => console.error('[boot] menu clear failed', e.message));

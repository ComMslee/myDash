...
function ymdKstNow(offsetDays = 0) {
...
async function cmdFestivals({ chatId, args }) {
...
async function showFestivals(chatId, { range, areaCode }) {
  let from, to;
  if (range === 'weekend') {
...
// /setarea [지역명|코드|전국]
async function cmdSetArea({ chatId, args }) {
  const arg = (args || '').trim();
  if (!arg) {

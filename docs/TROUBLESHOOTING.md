...
- URL: `http://<host>/v2/dev/api-status` (URL 직접 입력 — 하단 알약 미노출)
- 6개 카테고리 37개 라우트 + 시스템/충전 감지/폴링 진단 통합 뷰
- "전체 재실행" → 모든 ROUTES 동시 호출, 각 행 ▾ 펼침으로 raw peek + (해당되면) 대시보드 뷰
...
| "충전 안 잡힘", "처음엔 됐다 안 됨" (`charges` 스키마 함정) | `/api/charging-status` | `charging` / `pwr` / `lvl` / `pSig` / `lSig` |

...
`lib/dash-agg.js::bootstrapIfEmpty(carId)` 는 4종 사전집계(`dash_daily_*`, `dash_monthly_insights`, `dash_top_drives_cache`, `dash_place_clusters`) 가 비어 있을 때만 전체 히스토리 백필. 컨테이너 라이프타임당 1회만 돌고 `bootstrapInflight` Promise dedup 으로 동시 요청 안전. **첫 요청 10~60초** 지연 가능 — 배포 직후 `/api/insights` 등이 늦으면 의심.

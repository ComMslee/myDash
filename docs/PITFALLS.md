...
10. [`withCache` 콜백 내부 `Response.json` 중첩](#withcache-콜백-내부-responsejson-중첩) — 직렬화 2중으로 응답이 문자열화
11. [`bootstrap-on-empty` 콜드 스타트 지연](#bootstrap-on-empty-콜드-스타트-지연) — 첫 요청 10~60초 + 멱등성 오해
...
## `withCache` 콜백 내부 `Response.json` 중첩

`dashboard/lib/server-cache.js` 의 `withCache(key, ttl, fn)` 콜백은 **plain 객체만 반환**해야 함. 콜백 안에서 `return Response.json(payload)` 를 하면 첫 호출에서는 캐시에 `Response` 인스턴스가 박힌 채로 한 번 동작하지만, 라우트 레벨의 외부 래퍼 `return Response.json(await withCache(...))` 가 그것을 다시 직렬화하면서 `'{"key":"value"}'` 같은 문자열 페이로드가 응답으로 나가 클라이언트 파싱 깨짐. 라우트 패턴 고정: 콜백은 plain 객체, 직렬화는 외부에서 1회 — `Response.json(await withCache(key, ttl, async () => ({ ... })))` (`f4f7019`).

## `bootstrap-on-empty` 콜드 스타트 지연

`lib/dash-agg/bootstrap.js::bootstrapIfEmpty(carId)` 는 4종 사전집계(`dash_daily_*`, `dash_monthly_insights`, `dash_top_drives_cache`, `dash_place_clusters`) 가 비어 있을 때만 전체 히스토리 백필. 컨테이너 라이프타임당 1회만 돌고 `bootstrapInflight` Promise dedup 으로 동시 요청 안전. **첫 요청 10~60초** 지연 가능 — 배포 직후 `/api/insights` 등이 늦으면 의심.

**멱등성 오해**: cron 실패 회복용 수동 `POST /api/admin/refresh-aggs?scope=all` 호출 시, 이미 채워진 테이블이면 bootstrap 분기는 스킵된다. "전체 재계산" 을 강제하려면 scope 별 refresh 가 truncate-replace 인지(top/places는 truncate, daily/monthly 는 ON CONFLICT upsert) 확인 필요. 진짜 풀 리빌드가 필요하면 해당 테이블 `TRUNCATE` 후 컨테이너 재시작 → bootstrap 재발동 (`266c989`/`17d9727`).

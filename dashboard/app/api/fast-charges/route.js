...
export async function GET(request) {
  ...
  try {
    ...
    if (!car) return Response.json(isSummary ? { total_count: 0, total_kwh: 0, peak_kw: null } : { records: [], has_more: false });
    const carId = car.id;

    if (isSummary) {
      return Response.json(await withCache(`fast-charges-summary:${carId}`, TTL_180S, async () => {
        ...
        return {
          ...
          peak_kw: row.peak_kw ? parseFloat(parseFloat(row.peak_kw).toFixed(1)) : null,
        };
      }, { force }));
    }

    return Response.json(await withCache(`fast-charges:${carId}:${offset}`, TTL_180S, async () => {

...
// ── 이번달 인사이트 ───────────────────────────────────────────
function MonthInsightsCard({ drives }) {
  // drives API의 최근 목록에서 클라이언트 계산 (단순화)
  // drives 객체에 raw list가 없으면 null 반환
  ...
  // today/week 수치로 데이터 있음 여부 확인 — API 구조상 daily list 없으면 숨김
  const hasDrives = (drives.today_distance ?? 0) + (drives.week_distance ?? 0) > 0;
  if (!hasDrives) return null;
  ...
  return (
    <div className="bg-[#161618] border border-white/[0.06] rounded-2xl overflow-hidden">
      ...
      <div className="px-4 py-3 text-center text-zinc-600 text-xs">
        주행 이력 상세 데이터 연동 후 제공 예정
        <br />
        <span className="text-zinc-700">(러시아워 속도 · 장거리 비율 · 최장 미운행)</span>
      </div>
    </div>
  );
}
  ...
  return (
    <main className="min-h-screen bg-[#0f0f0f] text-white">
      <div className="max-w-2xl mx-auto px-4 py-5 pb-24 space-y-5">
        ...
        {/* 3. 이번달 인사이트 */}
        {!loading.drives && <MonthInsightsCard drives={drives} />}

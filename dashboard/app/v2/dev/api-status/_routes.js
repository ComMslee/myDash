...
export const CATEGORIES = ['차량', '주행/분석', '배터리/충전', '집충전기', '자동화/외부'];
...
export const ROUTES = [
  ...
  { path: '/api/monthly-history',  label: '월간 이력',      desc: '월별 주행거리/충전량/효율 집계 (cache 300s · dash_monthly_insights 위임)', category: '차량',
    params: [REFRESH_PARAM] },

  // 주행/분석 — 경로·히트맵·빈도·랭킹
  { path: '/api/route-map',        label: '경로 지도',      desc: '단일 주행의 polyline + start/end + 통계 (driveId 필수)', category: '주행/분석',
    params: [
      { key: 'driveId', required: true, sample: 'auto:firstDriveId' },
      { key: 'detail',  sample: '' },
    ] },
  { path: '/api/heatmap',          label: '히트맵',         desc: '전체 위치 좌표 다운샘플링 → 빈도 히트맵 입력 (cache 300s)', category: '주행/분석' },
  { path: '/api/frequent-places',  label: '자주 가는 곳',   desc: '지오펜스 도착 빈도 + 카카오 reverse geocode (집/회사 우선 핀) (cache 300s · dash_place_clusters 위임)', category: '주행/분석',
    params: [REFRESH_PARAM] },
  { path: '/api/long-stay-places', label: '오래 머문 곳',   desc: '체류 시간(다음 주행 시작-종료 갭) 누적 — 10분 미만 노이즈 필터 (cache 300s)', category: '주행/분석' },
  { path: '/api/rankings',         label: '랭킹',           desc: '주행/일자별 TOP N (type=거리·시간·평속·효율) (cache 300s/type·limit · dash_top_drives_cache 위임)', category: '주행/분석',
    params: [
      { key: 'type',  sample: 'drive_distance' },
      { key: 'limit', sample: '30' },
      REFRESH_PARAM,
    ] },
  { path: '/api/resolve-address',  label: '좌표→주소',      desc: 'lat/lng → 한국어 라벨 (Kakao 역지오코딩, DB 캐시) — 봇 알림 주소 폴백', category: '주행/분석',
    params: [
      { key: 'lat', required: true, sample: '37.5665' },
      { key: 'lng', required: true, sample: '126.9780' },
    ] },

  // 배터리/충전 — SOC·충전 세션·디버그
  { path: '/api/battery',          label: '배터리',         desc: 'SOC 종합 — 용량·체류 분포·주간/월간 충방전·추정 잔여 (cache 180s)', category: '배터리/충전',
    params: [REFRESH_PARAM] },
  { path: '/api/range-radius',     label: '잔여 거리 반경',  desc: '현 위치 + 예상 주행거리(est_km, rated 폴백) × 0.75 → 편도/왕복 반경 (cache 60s) — /v2/battery 상단 지도', category: '배터리/충전' },
  { path: '/api/battery-trend',    label: '배터리 추이',    desc: 'SOC 시계열 (라인 차트용 다운샘플링) (cache 600s)', category: '배터리/충전',
    params: [REFRESH_PARAM] },
  { path: '/api/charging-status',  label: '충전 상태',      desc: '현재 충전 중 여부 + power/level 신호 + 폴백 진단', category: '배터리/충전', dashboard: 'charging' },
  { path: '/api/charges',          label: '충전 기록',      desc: '최근 충전 세션 목록 (시작 SOC → 종료 SOC, kWh, 위치)', category: '배터리/충전' },
  { path: '/api/charge-all-time',  label: '충전 전기간',    desc: '전기간 누적 충전 통계 (총 kWh, 횟수, 평균) (cache 600s · dash_daily_charge_agg 단독)', category: '배터리/충전',
    params: [REFRESH_PARAM] },
  ...
  // 자동화/외부 — 스케줄러 + Tesla Fleet + 외부 API + 가족
  { path: '/api/schedules',            label: '스케줄 목록',       desc: '등록된 자동화 스케줄 + 활성/조건/마지막 실행', category: '자동화/외부' },
  { path: '/api/schedules/executions', label: '스케줄 이력',       desc: '실행 이력 통합 (limit 최대 500). status: success/failed/skipped/dry_run', category: '자동화/외부',
    params: [{ key: 'limit', sample: '50' }] },
  { path: '/api/automation-enabled',   label: '자동화 마스터 스위치', desc: '전체 ON/OFF — OFF 시 워커가 스케줄 평가/실행 스킵 (geofence 기록은 유지)', category: '자동화/외부' },
  ...
  { path: '/api/family/festivals', label: '축제',           desc: '한국관광공사 TourAPI(searchFestival2) 래핑 — 봇 /festivals (가족)', category: '자동화/외부',
    params: [
      { key: 'from',     sample: '' },
      { key: 'to',       sample: '' },
      { key: 'areaCode', sample: '' },
  ...
  { path: '/api/holidays',         label: '공휴일',         desc: 'KASI 특일정보 lazy 캐시 (dash_holidays · 30일 TTL) — 이력 리스트 일자 색상', category: '자동화/외부',
    params: [
      { key: 'year', sample: '' },
      REFRESH_PARAM,
    ] },

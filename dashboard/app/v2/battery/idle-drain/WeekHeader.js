  ...
  return (
    ...
    >
      <span className="flex items-center gap-2 min-w-0">
        <svg className={`w-4 h-4 text-zinc-500 flex-shrink-0 transition-transform ${expanded ? '' : '-rotate-90'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <span className="text-[10px] text-zinc-600">
          {weekClimatePct != null && (
            <span className="text-sky-700 mr-1 opacity-80" title={`공조 ${Math.round(weekClimateMin)}분`}>
              <Icon name="climate" className="w-4 h-4 inline-block align-middle mr-0.5" />{weekClimatePct}%
            </span>
          )}
          {weekSentryPct != null && (
            <span className="text-fuchsia-400 mr-1 opacity-80" title={`센트리 의심 ${Math.round(weekSentryMin)}분`}>
              <Icon name="shield" className="w-4 h-4 inline-block align-middle mr-0.5" />{weekSentryPct}%
            </span>
          )}
